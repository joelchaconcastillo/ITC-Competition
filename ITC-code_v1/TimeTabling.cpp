...
long long TimeTablingProblem::penalize_overall(int id_distribution, vector<vector<int>> &Graph_Hard_Constraints, vector<bool> &invalid_variables)
{
  ...
  if(dist.type == MAXDAYS)
  {
	...
	if( dist.required) cout << days << "-"<< id_class+1<<" *";
     } 
//cout<<endl;
     int nonzerobits = __builtin_popcountll(days);
    	if(dist.required &&  nonzerobits > dist.D )
	{
     	   for(int i = 1; i < class_c.size(); i++)
	   {
	//	Graph_Hard_Constraints[class_c[0]].push_back(class_c[i]);
...
//	cout << "maxbreaks... " << (Totalbreaks) << " " << dist.penalty<< " " <<(Totalbreaks*dist.penalty)/nrWeeks<< " "<< nrWeeks <<endl;
...
//            for(int i = 0; i < blocks.size(); i++)
//		{
//               if( ((end_b[i] - start_b[i]+1)  > dist.M)  && blocks[i].size() > 1)
//		cout << end_b[i] - start_b[i] <<" time "<< dist.required <<endl;
//		{
//		cout << end_b[i] << "|" <<start_b[i] <<"|"<<dist.M<<endl;
//		for(int j = 0; j < blocks[i].size(); j++)
//		cout << blocks[i][j]+1 << " " ;
//		cout <<endl;
//		}
...
//		      for(int j = 0; j < blocks[i].size(); j++)
//			Graph_Hard_Constraints[maxid].push_back(blocks[i][j]);
		//       invalid_variables[maxid] = true;
		       invalid_variables[blocks[i][blocks[i].size()-1]] = true;
                   }
                  TotalBlocksOver++;   
               }
            }    
...
//	    if( invalid_variables[id_room_i]) continue;
