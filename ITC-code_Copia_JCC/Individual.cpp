...
void Individual::iterated_forward_search_vns()
{
	...
	//optimize its neighbourhood
    //    for(int i = 0; i < TTP->dependency_var[idx].size(); i ++)
	//local_search_neighborhood(TTP->dependency_var[idx], current_indiv, i+1);
	local_search_neighborhood(TTP->dependency_var[idx], current_indiv, TTP->dependency_var[idx].size());
...
void Individual::local_search_neighborhood(vector<int> & variables,vector<pair<int, int>> &original_indiv, int Nvariables)
{
  ...
  while(cont++ < maxite)
  {
	...
	current_indiv[perm[i]] =  random_domain(perm[i]);
     ...
     long long current_f = mix_penalizations(incremental_evaluation(perm, current_indiv)); 
     long long best_f = mix_penalizations(incremental_evaluation(perm, best_indiv)); 
